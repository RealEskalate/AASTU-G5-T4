...
func (tc TrackController) CreateTrack(c *gin.Context) {
	...
	superGroupID := requestBody.SuperGroupID
	if track, err := tc.TrackUseCase.CreateTrack(context.TODO(), name, superGroupID); err != nil {
		c.JSON(500, gin.H{
...
func (tc TrackController) UpdateTrack(c *gin.Context) {
	...
	name := requestBody.Name
	if track, err := tc.TrackUseCase.UpdateTrack(context.TODO(), name, idInt); err != nil {
		c.JSON(500, gin.H{
