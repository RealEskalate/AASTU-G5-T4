...
const NavItem = ({ href, icon: Icon, label, isActive, hasSubmenu }: NavItemProps) => {
...
export function Sidebar({ userRole = "student", userName = "User", userTitle = "", userImage }: SidebarProps) {
  ...
  return (
    <aside
      className={`w-[220px] h-screen border-r dark:border-slate-700 bg-white dark:bg-slate-900 flex flex-col sidebar ${
        ...
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
            {userImage ? (
              ...
              <div className="h-full w-full flex items-center justify-center bg-theme text-white">
                {userName.charAt(0)}
              </div>
            )}
          </div>
      ...
      <div className="flex-1 overflow-auto py-4 px-2 space-y-6">
        ...
        <div>
          ...
          <nav className="space-y-1">
            ...
            <NavItem href="/tracks" icon={BookOpen} label="Tracks" isActive={pathname.includes("/tracks")} hasSubmenu />
            ...
            <NavItem href="/groups" icon={Map} label="Roadmap" isActive={pathname.includes("/roadmap")} />
            <NavItem href="/groups" icon={Users} label="Groups & Users" isActive={pathname.includes("/groups")} />
            <NavItem href="/forum" icon={MessageSquare} label="Forum" isActive={pathname.includes("/forum")} />
            <NavItem href="/events" icon={Calendar} label="Events" isActive={pathname.includes("/events")} />
          </nav>
        </div>
